...
pub trait TileEcsCommandExt<'w, 's, const N: usize> {
...
impl<'w, 's, const N: usize> TileEcsCommandExt<'w, 's, N> for Commands<'w, 's> {
    ...
    /// Despawns a tile.
    fn despawn_tile(&mut self, map_id: Entity, tile_c: [i32; N]) -> &mut Self {
        self.queue(DespawnTile::<N> { map_id, tile_c });
        self
    }
