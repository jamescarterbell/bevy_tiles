...
use bevy_tiles::tiles::{TileCoord, TileMapLabel};

use crate::prelude::{TilemapRenderSettings, TilemapTexture};
use crate::render::{
...
fn removal_helper(mut commands: Commands, mut removed_query: RemovedComponents<TileCoord>) {
    for entity in removed_query.iter() {
        commands.spawn(RemovedTileEntity(entity));
    }
}

fn removal_helper_tilemap(mut commands: Commands, mut removed_query: RemovedComponents<TileMap>) {
    for entity in removed_query.iter() {
